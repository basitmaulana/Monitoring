...
def kirim_push_landing(entri):
    ...
    payload = json.dumps({"title": judul, "body": isi, "nomor": entri["nomor"]})
    ...
    for sub in entri.get("push_subscriptions", []):
        try:
            ...
            print("Push error:", e)
            sisa.append(sub)  # error jaringan sesaat, jangan langsung dibuang
