...
function renderFlights() {
  ...
  state.flights.forEach(function (f) {
    ...
    if (f.status === "landed") {
      ...
      var catatanSumber = f.tiba_live
        ? ""
        : '<div class="flight-card__catatan">berdasarkan jadwal, belum live</div>';
...
// ---------- notifikasi push ----------
var KUNCI_STANDBY = "flightmon_standby";

function statusStandbyAktif() {
  var nilai = localStorage.getItem(KUNCI_STANDBY);
  return nilai === null ? true : nilai === "1"; // default: nyala
}

function simpanStatusStandby(aktif) {
  localStorage.setItem(KUNCI_STANDBY, aktif ? "1" : "0");
}
...
// ---------- toggle notifikasi standby ----------
var toggleStandbyEl = document.getElementById("toggle-standby");
toggleStandbyEl.checked = statusStandbyAktif();

toggleStandbyEl.addEventListener("change", function () {
  var aktif = toggleStandbyEl.checked;
  simpanStatusStandby(aktif);

  // kalau notifikasi sudah diizinkan, sinkronkan preferensi ini ke server
  // buat SEMUA flight yang lagi dipantau (bukan cuma yang baru ditambah)
  if (Notification.permission === "granted") {
    daftarkanPushKeSemuaFlightAktif();
  }

  toast(aktif
    ? "Notifikasi standby dinyalakan -- status bar bakal update terus."
    : "Notifikasi standby dimatikan -- cuma dikabari pas landing/dibatalkan."
  );
});
